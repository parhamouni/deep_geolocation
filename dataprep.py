...
import os
import sys
address = os.path.join(os.getcwd(), 'geographconv')
...
class DataLoader():
    def __init__(self, data_home, bucket_size=50, encoding='iso-8859-1',
    ...
    def load_data(self):
        data_address = os.path.join(self.data_home)
        logging.info('loading the dataset from %s' % data_address)
        train_file = os.path.join(data_address, 'user_info.train.gz')
        dev_file = os.path.join(data_address, 'user_info.dev.gz')
        test_file = os.path.join(data_address, 'user_info.test.gz')
    ...
    def get_graph(self):
        ...
        for i in range(len(self.df_train)):
            ...
            try:
                ...
                print('m is ' + str(m) + ' and i is ' + str(i))
            idmentions = set()
            for m in mentions:
                if m in node_id:
    ...
    def doc2vec(self, args):
        ...
        modelname = 'doc2vec_win_' + str(args.d2vwindow) + '_dm_' + str(args.d2vdm)
        address = os.path.join(self.data_home, modelname)
        if os.path.exists(address):
            model = Doc2Vec.load(address)
        else:
            logging.info('do2vec training started')
            documents_train = [TaggedDocument(token_pattern.tokenize(doc), [i]) for i, doc in
                               enumerate(self.df_train.text)]
            model = Doc2Vec(documents_train, vector_size=args.d2vec, window=args.d2vwindow, min_count=1, workers=4, dm=args.d2vdm)
        ...
        self.X_train_doc2vec = np.array([model.infer_vector(token_pattern.tokenize(row.text)) for user, row in
                          self.df_train.iterrows()])
        self.X_test_doc2vec = np.array([model.infer_vector(token_pattern.tokenize(row.text)) for user, row in
                          self.df_test.iterrows()])
        self.X_dev_doc2vec = np.array([model.infer_vector(token_pattern.tokenize(row.text)) for user, row in
                          self.df_dev.iterrows()])
    ...
    def word2vec(self,args): ##TODO word2vec implementation + with continous bag of word adding the args keys to all the experiments w2vwindow,  vector_size w2vvec, cbow_mean
            ## this failed because it doesnt provide a fixed sized presentation for each user. It depends on the number of words in each timeline
            # training doc2vec on training set
            token_pattern = RegexpTokenizer(self.token_pattern)
            modelname = 'word2vec_win_' + str(args.w2vwindow) + '_v_' +  str(args.w2vvec) + '_cbow_mean' + str(args.cbow_mean*1) + '.wv'
            address = os.path.join(self.data_home, modelname)

            if os.path.exists(address):
                model = Word2Vec.load(address)
            else:
                logging.info('word2vec training started')
                documents_train = [token_pattern.tokenize(doc) for doc in
                            self.df_train.text]
                model = Word2Vec(documents_train, size= args.w2vvec , window=args.w2vwindow, min_count=1, workers=4, cbow_mean=args.cbow_mean*1)
                model.save(address)
            ...
            for user, row in self.df_train.iterrows():
                try:
                    train.append(np.array(model.wv[token_pattern.tokenize(row.text)]))
                except KeyError as K:
                    print(K)
                    train.append(np.array(model.wv['unknown']))
            test=[]
            for user, row in self.df_test.iterrows():
                try:
                    test.append(model.wv[token_pattern.tokenize(row.text)])
                except KeyError as K:

                    test.append(model.wv['unknown'])
            dev = []
            for user, row in self.df_dev.iterrows():
                try:
                    dev.append(model.wv[token_pattern.tokenize(row.text)])
                except KeyError as K:
                    dev.append(model.wv['unknown'])


            self.X_train_word2vec = np.array(train)


            np.save('word2vecembedding',self.X_train_word2vec)

            self.X_train_word2vec  = np.reshape(self.X_train_word2vec, (-1, args.w2vvec))
            print(self.X_train_word2vec.shape)


            self.X_test_word2vec = np.array(test)
            print(self.X_test_word2vec.shape)

            self.X_dev_word2vec = np.array(dev)

            print(self.X_dev_word2vec.shape)
    ...
    def assignClasses(self):
        ...
        nnbr.fit(list(self.cluster_median.values()))
        ...
        if self.one_hot_labels:
            ...
            y_dev = np.zeros((len(self.dev_classes), num_weight_matrixlabels), dtype=np.float32)
...
if __name__ == '__main__':
    data_loader = DataLoader(data_home='./datasets/cmu')
