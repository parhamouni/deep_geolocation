...
class DataLoader():
    ...
    def doc2vec(self, args):
        ...
        if os.path.exists(address):
            ...
            model = Doc2Vec(documents_train, vector_size=300, window=args.d2vwindow, min_count=1, workers=4, dm=args.d2vdm)
        ...
        ## TODO check whether adjusted weights are needed in the experiments,I believe not
        weight_matrix = np.zeros((l,l), dtype=np.float16)
        ...
        predicted_similarities = predicted_similarities.astype(np.uint16)

        ## creating the adjacency matrix

        adj_doc2vec = np.zeros((l,l), dtype=np.float16)
