...
def main(data, args, **kwargs):
    ...
    concatenated_layer = X
    concatenated_layer = concatenated_layer.tocsr()

    dataset_name = args.dir.split('/')[-1]

    filename = 'doc2vec_results' + '_d_' + dataset_name + '_f_' + str(args.lblfraction[0]) + '_d2v_' + str(args.doc2vec * 1) + '_d2vw_' + str(args.d2vwindow) + '_d2vdm_' + str(args.d2vdm) +  ".txt"
    ...
    if not notoutput:
        ...
        results = pd.DataFrame(columns=['dataset','model_name', 'mean', 'median',
                                        'acc','score','fraction', 'd2v','window_size', 'd2vdm'])
        for name, clf in zip(names, classifiers):
            ...
            row = [dataset_name, name, mean, median, acc,score, args.lblfraction[0], args.doc2vec*1, window_size, d2vdm]
            results.loc[-1] = row
...
if __name__ == '__main__':
    ...
    main(data, args, save_results=args.not_save_results)
